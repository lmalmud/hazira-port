...
for resource in WEEKLY_PLANNED: # For each resource that must be maintained weekly
    ...
    for timestamp in pd.date_range('2025-01-01 00:00', '2025-12-31 23:00', freq='W'):
        ...
        # Hours since beginning of simulation that the maintenance will occur at
        total_hours = (maintenance_start - pd.to_datetime('2025-01-01 00:00')).total_seconds() / 3600

        maintenance_events.append(MaintenanceEvent(resource, total_hours, 3.5))
...
for resource in MONTHLY_CORRECTIVE:
    ...
    for timestamp in pd.date_range('2025-01-01 00:00', '2025-12-31 23:00', freq='MS'):
        ...
        for day in scheduled_days:
            ...
            # All events are measured in hours since simulation start
            total_hours = (maintenance_start - pd.to_datetime('2025-01-01 00:00')).total_seconds() / 3600

            # Add this event to list of all maintenance events
            maintenance_events.append(MaintenanceEvent(resource, total_hours, 4.5))
